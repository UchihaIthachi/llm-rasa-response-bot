...
class ActionLLMGenerateResponseCommon(Action):
    ...
    async def run(
        ...
        if not latest_query or not latest_intent:
            dispatcher.utter_message(template=f"utter_{latest_intent}")
        ...
        try:
            ...
            if llm_response:
                ...
                dispatcher.utter_message(template=f"utter_{latest_intent}")
                logger.info(
                    f"LLM returned an empty response. Falling back to the specific default response: utter_{latest_intent}"
                )
        except Exception as e:
            dispatcher.utter_message(template=f"utter_{latest_intent}")
            logger.exception(
                f"An exception occurred while generating the LLM response. "
                f"Falling back to the specific default response: utter_{latest_intent}. {e}"
            )
        return []
...
class ActionLLMGenerateResponseMenu(Action):
    ...
    async def run(
        ...
        if not latest_query:
            dispatcher.utter_message(response=DEFAULT_REPHRASE_RESPONSE)
